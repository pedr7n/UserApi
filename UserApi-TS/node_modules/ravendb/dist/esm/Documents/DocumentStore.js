...
export class DocumentStore extends DocumentStoreBase {
    _log = getLogger({ module: "DocumentStore-" + Math.floor(Math.random() * 1000) });
    ...
    getRequestExecutor(database) {
        ...
        const createRequestExecutorForSingleNode = () => {
            const forSingleNode = RequestExecutor.createForSingleNodeWithConfigurationUpdates(this.urls[0], database, {
                authOptions: this.authOptions,
                documentConventions: this.conventions
            });
    ...
    _createDatabaseChanges(node) {
        return new DatabaseChanges(this.getRequestExecutor(node.databaseName), node.databaseName, () => this._databaseChanges.delete(this._getDatabaseChangesCacheKey(node)), node.nodeTag);
    }
    ...
    bulkInsert(databaseOrOptions, optionalOptions) {
        ...
        const options = TypeUtil.isString(databaseOrOptions) ? optionalOptions : databaseOrOptions;
