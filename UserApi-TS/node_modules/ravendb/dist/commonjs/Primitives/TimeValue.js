...
class TimeValue {
    ...
    static SECONDS_IN_28_DAYS = 28 * TimeValue.SECONDS_PER_DAY; // lower-bound of seconds in month
    static SECONDS_IN_31_DAYS = 31 * TimeValue.SECONDS_PER_DAY; // upper-bound of seconds in month
    static SECONDS_IN_365_DAYS = 365 * TimeValue.SECONDS_PER_DAY; // lower-bound of seconds in a year
    static SECONDS_IN_366_DAYS = 366 * TimeValue.SECONDS_PER_DAY; // upper-bound of seconds in a year
    static ZERO = new TimeValue(0, "None");
    static MAX_VALUE = new TimeValue(TypeUtil_js_1.TypeUtil.MAX_INT32, "None");
    static MIN_VALUE = new TimeValue(TypeUtil_js_1.TypeUtil.MIN_INT32, "None");
    _value;
    _unit;
    get value() {
        return this._value;
    ...
    toString() {
        if (this._value === TypeUtil_js_1.TypeUtil.MAX_INT32) {
            return "MaxValue";
        }
        if (this._value === TypeUtil_js_1.TypeUtil.MIN_INT32) {
            return "MinValue";
        }
        if (this._value === 0) {
            return "Zero";
        ...
        const str = new StringBuilder_js_1.StringBuilder();
        switch (this._unit) {
            case "Second": {
            ...
            default: {
                (0, index_js_1.throwError)("InvalidArgumentException", "Not supported unit: " + this._unit);
            }
        }
    ...
    _assertSeconds() {
        if (this._unit !== "Second") {
            (0, index_js_1.throwError)("InvalidArgumentException", "The value must be seconds");
        }
    }
    static _assertValidUnit(unit) {
        ...
        (0, index_js_1.throwError)("InvalidArgumentException", "Invalid time unit: " + unit);
    }
    static _assertSameUnits(a, b) {
        if (a.unit !== b.unit) {
            (0, index_js_1.throwError)("InvalidArgumentException", "Unit isn't the same " + a.unit + " != " + b.unit);
        }
    }
    compareTo(other) {
        ...
        (0, index_js_1.throwError)("InvalidOperationException", "Unable to compare " + this + " with " + other + ", since a month might have different number of days.");
    }
    static _getBoundsInSeconds(time) {
        switch (time._unit) {
            ...
            default: {
                (0, index_js_1.throwError)("InvalidArgumentException", "Not supported time value unit: " + time._unit);
            }
        }
    }
    ...
    static _isMax(time) {
        return time._unit === "None" && time._value >= TypeUtil_js_1.TypeUtil.MAX_INT32;
    }
    static _isMin(time) {
        return time._unit === "None" && time._value <= TypeUtil_js_1.TypeUtil.MIN_INT32;
    }
    static _trimCompareResult(result) {
        if (result > TypeUtil_js_1.TypeUtil.MAX_INT32) {
            return TypeUtil_js_1.TypeUtil.MAX_INT32;
        }
        if (result < TypeUtil_js_1.TypeUtil.MIN_INT32) {
            return TypeUtil_js_1.TypeUtil.MIN_INT32;
        }
        return result;
    }
    serialize() {
