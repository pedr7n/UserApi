...
class GetDocumentsCommand extends RavenCommand_js_1.RavenCommand {
    _id;
    _ids;
    _includes;
    _counters;
    ...
    constructor(opts) {
        ...
        if (opts.hasOwnProperty("id")) {
            ...
            if (!opts.ids || opts.ids.length === 0) {
                (0, index_js_1.throwError)("InvalidArgumentException", "Please supply at least one id");
            }
            this._ids = opts.ids;
            this._includes = opts.includes;
            this._metadataOnly = opts.metadataOnly;
            ...
            if (opts.hasOwnProperty("startsWith")) {
                if (!opts.startsWith) {
                    (0, index_js_1.throwError)("InvalidArgumentException", "startWith cannot be null");
                }
        ...
        if (opts.hasOwnProperty("counterIncludes")) {
            ...
            if (!counters) {
                (0, index_js_1.throwError)("InvalidArgumentException", "CounterIncludes cannot be null.");
            }
    ...
    createRequest(node) {
        ...
        if (this._includeAllCounters) {
            query += `&counter=${Constants_js_1.COUNTERS.ALL}`;
        }
            ...
            for (const tsInclude of this._timeSeriesIncludes) {
                if ("from" in tsInclude) {
                    ...
                    query += "&timeseries=" + this._urlEncode(range.name)
                        + "&from=" + (range.from ? DateUtil_js_1.DateUtil.utc.stringify(range.from) : "")
                        + "&to=" + (range.to ? DateUtil_js_1.DateUtil.utc.stringify(range.to) : "");
                }
                    ...
                    (0, index_js_1.throwError)("InvalidArgumentException", "Unexpected TimeSeries range: " + tsInclude);
                }
            }
        }
        ...
        if (this._revisionsIncludeByDateTime) {
            query += "&revisionsBefore=" + this._urlEncode(DateUtil_js_1.DateUtil.utc.stringify(this._revisionsIncludeByDateTime));
        }
