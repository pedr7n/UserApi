...
const ObjectMapper_js_1 = require("../../Mapping/ObjectMapper.js");
...
const BulkInsertConventions_js_1 = require("./BulkInsertConventions.js");
const ShardingConventions_js_1 = require("./ShardingConventions.js");
const pluralize_js_1 = require("../../ext/pluralize/pluralize.js");
...
class DocumentConventions {
    ...
    constructor() {
        ...
        this._identityProperty = Constants_js_1.CONSTANTS.Documents.Metadata.ID_PROPERTY;
        this._findJsType = (id, doc) => {
            const metadata = doc[Constants_js_1.CONSTANTS.Documents.Metadata.KEY];
            if (metadata) {
                const jsType = metadata[Constants_js_1.CONSTANTS.Documents.Metadata.RAVEN_JS_TYPE];
        ...
        this._bulkInsert = new BulkInsertConventions_js_1.BulkInsertConventions(() => this._assertNotFrozen());
        this._sharding = new ShardingConventions_js_1.ShardingConventions(this);
        this._maxHttpCacheSize = 128 * 1024 * 1024;
        this._knownEntityTypes = new Map();
        this._objectMapper = new ObjectMapper_js_1.TypesAwareObjectMapper({
            documentConventions: this
        });
        this._dateUtilOpts = {};
        this._dateUtil = new DateUtil_js_1.DateUtil(this._dateUtilOpts);
    ...
    deserializeEntityFromJson(documentType, document) {
        try {
            ...
            (0, index_js_1.throwError)("RavenException", "Cannot deserialize entity", err);
        }
    }
    ...
    set identityPartsSeparator(value) {
        ...
        if (this.identityPartsSeparator === "|") {
            (0, index_js_1.throwError)("InvalidArgumentException", "Cannot set identity parts separator to '|'");
        }
    ...
    static defaultGetCollectionName(ctorOrTypeChecker) {
        ...
        if (!TypeUtil_js_1.TypeUtil.isObjectTypeDescriptor(ctorOrTypeChecker)) {
            (0, index_js_1.throwError)("InvalidArgumentException", "Invalid class argument.");
        }
        if (!ctorOrTypeChecker.name) {
            (0, index_js_1.throwError)("InvalidArgumentException", "Type name cannot be null or undefined.");
        }
        ...
        if (typeof (ctorOrTypeChecker) === "string") {
            result = (0, pluralize_js_1.plural)(ctorOrTypeChecker);
        }
        else {
            result = (0, pluralize_js_1.plural)(ctorOrTypeChecker.name);
        }
    ...
    _assertNotFrozen() {
        if (this._frozen) {
            (0, index_js_1.throwError)("RavenException", "Conventions has been frozen after documentStore.initialize() and no changes can be applied to them");
        }
    }
    ...
    registerEntityType(entityType, name) {
        if (!TypeUtil_js_1.TypeUtil.isObjectTypeDescriptor(entityType)) {
            (0, index_js_1.throwError)("InvalidArgumentException", "Entity type must be a constructor or an object literal descriptor.");
        }
        if (name) {
    ...
    tryRegisterEntityType(docType) {
        if (TypeUtil_js_1.TypeUtil.isObjectTypeDescriptor(docType)) {
    ...
    transformObjectKeysToRemoteFieldNameConvention(obj) {
        ...
        return ObjectUtil_js_1.ObjectUtil.transformObjectKeys(obj, options);
    }
    validate() {
        if ((this._localToServerFieldNameConverter && !this._serverToLocalFieldNameConverter)
            ...
            (0, index_js_1.throwError)("ConfigurationException", "When configuring field name conventions, "
