...
class TimeSeriesIncludesToken extends QueryToken_js_1.QueryToken {
    _sourcePath;
    _range;
    ...
    addAliasToPath(alias) {
        this._sourcePath = StringUtil_js_1.StringUtil.isNullOrEmpty(this._sourcePath)
            ? alias
    ...
    writeTo(writer) {
        writer
            .append("timeseries(");
        if (!StringUtil_js_1.StringUtil.isNullOrEmpty(this._sourcePath)) {
            writer
                .append(this._sourcePath)
                .append(", ");
        }
        if (!StringUtil_js_1.StringUtil.isNullOrEmpty(this._range.name)) {
            writer
                .append("'")
        ...
        if ("count" in this._range) {
            ...
            (0, index_js_1.throwError)("InvalidArgumentException", "Not supported time range type: " + this._range);
        }
        writer
            .append(")");
    }
    static _writeTimeRangeTo(writer, range) {
        switch (range.type) {
            ...
            default: {
                (0, index_js_1.throwError)("InvalidArgumentException", "Not supported time range type: " + range.type);
            }
        }
        writer
    ...
    static _writeRangeTo(writer, range) {
        if (range.from) {
            writer
                .append("'")
                .append(DateUtil_js_1.DateUtil.utc.stringify(range.from))
                .append("'")
                .append(", ");
        }
        else {
        ...
        if (range.to) {
            writer
                .append("'")
                .append(DateUtil_js_1.DateUtil.utc.stringify(range.to))
                .append("'");
        }
        else {
            writer
...
//# sourceMappingURL=TimeSeriesIncludesToken.js.map