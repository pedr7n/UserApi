...
const ClusterWideBatchCommand_js_1 = require("../../Commands/Batches/ClusterWideBatchCommand.js");
const SingleNodeBatchCommand_js_1 = require("../../Commands/Batches/SingleNodeBatchCommand.js");
class BatchOperation {
    ...
    createRequest() {
        ...
        if (this._session.transactionMode === "ClusterWide") {
            return new ClusterWideBatchCommand_js_1.ClusterWideBatchCommand(this._session.conventions, result.sessionCommands, result.options, this._session.disableAtomicDocumentWritesInClusterWideTransaction);
        }
        return new SingleNodeBatchCommand_js_1.SingleNodeBatchCommand(this._session.conventions, result.sessionCommands, result.options);
    }
    static _throwOnNullResults() {
        (0, index_js_1.throwError)("InvalidOperationException", "Received empty response from the server. This is not supposed to happen and is likely a bug.");
    }
    setResult(result) {
        if (!result.results) {
        ...
        if (this._session.transactionMode === "ClusterWide") {
            if (result.transactionIndex <= 0) {
                (0, index_js_1.throwError)("ClientVersionMismatchException", "Cluster transaction was send to a node that is not supporting it. "
        ...
        for (let i = 0; i < this._sessionCommandsCount; i++) {
            ...
            switch (type) {
                ...
                default: {
                    (0, index_js_1.throwError)("InvalidOperationException", `Command '${type}' is not supported.`);
                }
            }
        }
    ...
    _applyMetadataModifications(id, documentInfo) {
        ...
        documentInfo.metadata = ObjectUtil_js_1.ObjectUtil.deepLiteralClone(documentInfo.metadata);
        documentInfo.metadata["@change-vector"] = documentInfo.changeVector;
        const documentCopy = ObjectUtil_js_1.ObjectUtil.deepLiteralClone(documentInfo.document);
        documentCopy[Constants_js_1.CONSTANTS.Documents.Metadata.KEY] = documentInfo.metadata;
        documentInfo.document = documentCopy;
    }
    _getOrAddModifications(id, documentInfo, applyModifications) {
        if (!this._modifications) {
            this._modifications = CaseInsensitiveKeysMap_js_1.CaseInsensitiveKeysMap.create();
        }
    ...
    _handleAttachmentDelete(batchResult) {
        this._handleAttachmentDeleteInternal(batchResult, "AttachmentDELETE", Constants_js_1.CONSTANTS.Documents.Metadata.ID, "name", "documentChangeVector");
    }
    ...
    _handlePatch(batchResult) {
        ...
        switch (status) {
            ...
            case "Patched": {
                ...
                documentInfo.metadata[Constants_js_1.CONSTANTS.Documents.Metadata.ID] = id;
                documentInfo.metadata[Constants_js_1.CONSTANTS.Documents.Metadata.CHANGE_VECTOR] = changeVector;
                documentInfo.metadata[Constants_js_1.CONSTANTS.Documents.Metadata.LAST_MODIFIED] = lastModified;
                ...
                if (documentInfo.entity) {
                    ...
                    const afterSaveChangesEventArgs = new SessionEvents_js_1.SessionAfterSaveChangesEventArgs(this._session, documentInfo.id, documentInfo.entity);
    ...
    _handleForceRevisionCreation(batchResult) {
        ...
        const id = BatchOperation._getStringField(batchResult, "ForceRevisionCreation", Constants_js_1.CONSTANTS.Documents.Metadata.ID);
        const changeVector = BatchOperation._getStringField(batchResult, "ForceRevisionCreation", Constants_js_1.CONSTANTS.Documents.Metadata.CHANGE_VECTOR);
        const documentInfo = this._session.documentsById.getValue(id);
        if (!documentInfo) {
            return;
        }
    ...
    _handlePut(index, batchResult, isDeferred) {
        ...
        const id = BatchOperation._getStringField(batchResult, "PUT", Constants_js_1.CONSTANTS.Documents.Metadata.ID);
        const changeVector = BatchOperation._getStringField(batchResult, "PUT", Constants_js_1.CONSTANTS.Documents.Metadata.CHANGE_VECTOR);
        if (isDeferred) {
            const sessionDocumentInfo = this._session.documentsById.getValue(id);
            if (!sessionDocumentInfo) {
                return;
    ...
    static _getStringField(json, type, fieldName, throwOnMissing = true) {
        if ((!(fieldName in json) || TypeUtil_js_1.TypeUtil.isNullOrUndefined(json[fieldName])) && throwOnMissing) {
            BatchOperation._throwMissingField(type, fieldName);
        }
        const jsonNode = json[fieldName];
        if (jsonNode && !TypeUtil_js_1.TypeUtil.isString(jsonNode)) {
            (0, index_js_1.throwError)("InvalidOperationException", `Expected response field ${fieldName} to be a string.`);
        }
        return jsonNode;
    }
    ...
    static _throwInvalidValue(arg, fieldName) {
        (0, index_js_1.throwError)("InvalidOperationException", "'" + arg + "' is not a valid value for field " + fieldName);
    }
    static _throwMissingField(type, fieldName) {
        (0, index_js_1.throwError)("InvalidOperationException", type + " response is invalid. Field '" + fieldName + "' is missing.");
    }
}
