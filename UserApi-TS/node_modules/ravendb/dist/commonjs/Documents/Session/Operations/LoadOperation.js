...
const log = (0, LogUtil_js_1.getLogger)({ module: "LoadOperation" });
class LoadOperation {
    _session;
    _ids;
    _includes;
    ...
    createRequest() {
        ...
        const cmd = new GetDocumentsCommand_js_1.GetDocumentsCommand(opts);
    ...
    withRevisions(revisions) {
        if (TypeUtil_js_1.TypeUtil.isArray(revisions)) {
            if (revisions.length > 0) {
                this._session.assertNoIncludesInNonTrackingSession();
                this._revisionsToIncludeByChangeVector = revisions;
            }
        }
        if (TypeUtil_js_1.TypeUtil.isDate(revisions)) {
            this._session.assertNoIncludesInNonTrackingSession();
    ...
    byIds(ids) {
        const distinct = new Set(ids.filter(x => !StringUtil_js_1.StringUtil.isNullOrEmpty(x)));
    ...
    getDocument(clazz) {
        if (this._session.noTracking) {
            if (!this._resultsSet && this._ids.length) {
                (0, index_js_1.throwError)("InvalidOperationException", "Cannot execute getDocument before operation execution.");
            }
            ...
            const documentInfo = DocumentInfo_js_1.DocumentInfo.getNewDocumentInfo(document);
    ...
    setResult(result) {
        ...
        for (const document of result.results) {
            if (!document || TypeUtil_js_1.TypeUtil.isNullOrUndefined(document)) {
                continue;
            }
            const newDocumentInfo = DocumentInfo_js_1.DocumentInfo.getNewDocumentInfo(document);
